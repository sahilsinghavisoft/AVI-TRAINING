...
CREATE TABLE employees (EMPLOYEE_ID INT PRIMARY KEY,FIRST_NAME VARCHAR(50),LAST_NAME VARCHAR(50),EMAIL VARCHAR(100),
    PHONE_NUMBER VARCHAR(20),HIRE_DATE DATE,JOB_ID VARCHAR(10),SALARY DECIMAL(10,2),COMMISSION_PCT DECIMAL(4,2),
...
(172, 'Elizabeth', 'Bates', 'EBATES', '011.44.1343.529268', '1987-08-28', 'SA_REP', 7300.00, 0.15, 148, 80),
(173, 'Sundita', 'Kumar', 'SKUMAR', '011.44.1343.329268', '1987-08-29', 'SA_REP', 6100.00, 0.10, 148, 80),
(174, 'Emily', 'Johnson', 'EJOHNSON', '011.44.1343.229268', '1987-08-28', 'SA_REP', 8200.00, 0.10, 148, 80),
(175, 'Samuel', 'Doe', 'SDOE', '011.44.1343.129268', '1987-08-29', 'SA_REP', 7900.00, 0.10, 148, 80),
(176, 'Rachel', 'White', 'RWHITE', '011.44.1343.029268', '1987-08-30', 'SA_REP', 7500.00, 0.10, 148, 80),
(177, 'Jack', 'Livingston', 'JLIVINGS', '011.44.1644.429264', '1987-09-02', 'SA_REP', 8400.00, 0.20, 149, 80),
(178, 'Kimberely', 'Grant', 'KGRANT', '011.44.1644.429263', '1987-09-03', 'SA_REP', 7000.00, 0.15, 149, 0),
(179, 'Charles', 'Johnson', 'CJOHNSON', '011.44.1644.429262', '1987-09-04', 'SA_REP', 6200.00, 0.10, 149, 80),
(180, 'Winston', 'Taylor', 'WTAYLOR', '650.507.9876', '1987-09-05', 'SH_CLERK', 3200.00, 0.00, 120, 50),
...
(203, 'Susan', 'Mavris', 'SMAVRIS', '515.123.7777', '1987-09-28', 'HR_REP', 6500.00, 0.00, 101, 40),
(204, 'Hermann', 'Baer', 'HBAER', '515.123.8888', '1987-09-29', 'PR_REP', 10000.00, 0.00, 101, 70),
(205, 'Shelley', 'Higgins', 'SHIGGINS', '515.123.8080', '1987-09-30', 'AC_MGR', 12008.00, 0.00, 101, 110),
(206, 'William', 'Gietz', 'WGIETZ', '515.123.8181', '1987-10-01', 'AC_ACCOUNT', 8300.00, 0.00, 205, 110);
...
VALUES (10, 'Administration', 200, 1700),
       ...
       (100, 'Finance', 108, 1700),
       (110, 'Accounting', 205, 1700),
       (120, 'Treasury', NULL, 1700),  -- Insert NULL for departments without a manager
...
SELECT d.DEPARTMENT_NAME, COUNT(*) AS number_of_employees FROM employees e  
JOIN departments d ON e.DEPARTMENT_ID = d.DEPARTMENT_ID GROUP BY d.DEPARTMENT_NAME;

/*
3.Write a MySQL query to find the employee ID, job title, number of days between ending date and starting date for all jobs in department 90 from job history.
...
CREATE TABLE job_history (
    employee_id INT,start_date DATE,end_date DATE,job_id VARCHAR(20),department_id INT
);
...
*/
SELECT d.DEPARTMENT_ID, d.DEPARTMENT_NAME, e.FIRST_NAME AS MANAGER_FIRST_NAME FROM departments d
...
CREATE TABLE locations (
    location_id INT PRIMARY KEY,street_address VARCHAR(255),postal_code VARCHAR(50),city VARCHAR(255),
    state_province VARCHAR(255),country_id VARCHAR(2)
);
...
SELECT d.DEPARTMENT_NAME, e.FIRST_NAME, l.city  FROM employees e 
...
/*
5.5.Write a MySQL query to display the job title and average salary of employees.
...
Stock Clerk					2785
Stock Manager					7280*/
SELECT j.JOB_TITLE, AVG(e.SALARY) AS AVG_SALARY FROM employees e
